...
violations contains v if {
	not vsa_envelope
	v := "No VSA found in attestation"
}

violations contains v if {
	vsa_envelope
	not verified_statement(config.doi.keys, vsa_envelope)
	v := "VSA statement not correctly signed"
}

violations contains v if {
	vsa_envelope
	statement := verified_statement(config.doi.keys, vsa_envelope)
	not valid_vsa(statement)
	v := "VSA statement not right type"
}
